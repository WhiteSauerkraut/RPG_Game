...
public class Inventroy : MonoBehaviour {
    ...
    public Slot[] slotArray;//存放物品槽的数组
    ...
    public bool StoreItem(ItemDetail item) 
    {
        if (item.m_Id == 0)
        {
            Debug.LogWarning("要存储物品的m_Id不存在");
            return false;
        }

        if (item.m_Capacity == 1)//如果此物品只能放一个，那就找一个空的物品槽来存放即可
        {
    ...
    public void SaveInventory() 
    {
        ...
        foreach (Slot slot in slotArray)
        {
            if (slot.transform.childCount > 0 )
            {
                ...
                sb.Append(itemUI.ItemDetail.m_Id + "," + itemUI.Amount + "-");//用逗号分隔一个物品中的m_Id和数量，用 - 分隔多个物品
            }
            else
            {
    ...
    public void LoadInventory() 
    {
        ...
        for (int i = 0; i < itemArray.Length-1; i++)//长度减1是因为最后一个字符是 “-”，不需要取它
        {
            ...
            if (itemStr != "0")
            {
                string[] temp = itemStr.Split(',');//按照逗号分隔这个物品的信息（m_Id和Amoun数量）
                int id = int.Parse(temp[0]);
                ItemDetail item = InventroyManager.Instance.GetItemById(id);//通过物品m_Id得到该物品
