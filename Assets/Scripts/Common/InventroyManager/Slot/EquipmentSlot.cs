...
public class EquipmentSlot :Slot
{
    ...
    public override void OnPointerDown(UnityEngine.EventSystems.PointerEventData eventData)//重写父类Slot的方法
    {
        if (eventData.button == PointerEventData.InputButton.Right)//鼠标右键点击直接实现脱掉，不经过拖拽
        {
            if (transform.childCount > 0 && InventroyManager.Instance.IsPickedItem == false)//需要脱掉的物品得有，并且鼠标上要没有物品，否则就发生：当鼠标上有物品，在其他物品上点击鼠标右键也能脱掉这种情况。
            {
                ...
                ItemDetail item = currentItemUI.ItemDetail;//临时保存物品信息，防止下面一销毁就没了
        ...
        if (InventroyManager.Instance.IsPickedItem == true)//一：鼠标上有物品
        {
            ...
            if (transform.childCount > 0)//1.当前装备槽有装备
            {
                ...
                if (IsRightItem(pickedItemUI.ItemDetail) && pickedItemUI.Amount == 1)//如果鼠标上的物品是否适合放在该位置并且鼠标上的物品只有一个时，那就交换，否则不做处理
                {
            ...
            {
                if (IsRightItem(pickedItemUI.ItemDetail))//如果鼠标上的物品是否适合放在该位置，那就把该物品放入角色面板，并且鼠标上的物品减少一个，否则不做处理
                {
    ...
    {
        if (item is Equipment && ((Equipment)(item)).EquipType == this.equipmentSoltType)//如果此物品是装备并且此物品的装备类型和当前物品槽中的装备类型相同   或者   此物品是武器并且此武器的类型和当前物品槽中的武器类型相同，那就可以合适放在该位置，否则不合适。
        {
            return true;
        }
        else
